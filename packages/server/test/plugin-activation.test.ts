...
const fixture = Effect.fn(function* (plugin: Plugin.Plugin) {
  const tmp = yield* Effect.acquireRelease(
    Effect.promise(() => tmpdir("opencode-plugin-activation-")),
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(yield* Effect.promise(() => active.json())).toMatchObject({
        data: expect.arrayContaining([
          expect.objectContaining({ id: "slow-plugin", source: { type: "sdk" }, status: "active" }),
        ]),
      })
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(yield* Effect.promise(() => plugins.json())).toMatchObject({
        data: expect.arrayContaining([expect.objectContaining({ id: "slow-plugin", status: "active" })]),
      })
    }).pipe(Effect.timeout("10 seconds")),
  15_000,
)
