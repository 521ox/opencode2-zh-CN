...
it.live("updates completed assistant message content through the session HTTP API", () =>
  Effect.gen(function* () {
    const tmp = yield* Effect.acquireRelease(
      Effect.promise(() => tmpdir("session-message-update-")),
      (directory) => Effect.promise(() => directory[Symbol.asyncDispose]()),
    )
    const root = (name: string) => path.join(tmp.path, name)
    ...
    const execution = Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        return SessionExecution.Service.of({
          owner: { id: "session-message-update", pid: process.pid, hostname: "test", leaseMs: 30_000 },
          ...
          wake: (sessionID) =>
            Effect.gen(function* () {
              ...
              if (!state.complete) return { type: "owned" as const }
    ...
    const handler = yield* ServerFetch.make(
      {
        ...
        models: { fetch: false },
        config: { directory: root("config"), project: false, content: "{}" },
