...
it.live("updates package plugins in the requested Location", () =>
  Effect.gen(function* () {
    ...
    const entry = () => ({
      directory: plugin,
      entrypoint: pathToFileURL(path.join(plugin, "index.mjs")).href,
      version,
      revision: version,
    })
    ...
    const listedVersion = yield* Effect.promise(async () => {
      ...
      const body = Schema.decodeUnknownSync(Schema.Struct({ data: Schema.Array(Plugin.Info) }))(await response.json())
      const item = body.data.find((item) => item.id === "fixture.plugin")
      return item?.source.type === "package" ? item.source.version : undefined
    }).pipe(
      Effect.filterOrFail((value) => value === "2.0.0"),
