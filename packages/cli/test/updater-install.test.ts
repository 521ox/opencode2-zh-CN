...
function fixture(
  ...
  return Effect.gen(function* () {
    ...
    const executable = path.join(root, "package", "bin", "opencode2")
    yield* fs.makeDirectory(path.dirname(executable), { recursive: true })
    yield* fs.writeFileString(
      path.join(root, "package", "package.json"),
      JSON.stringify({ name, bin: { opencode2: "bin/opencode2" } }),
    )
    yield* Effect.acquireRelease(
      Effect.sync(() =>
        spyOn(globalThis, "fetch").mockImplementation(
          Object.assign(async () => Response.json({ version: "2.3.4", metadata: { package: releaseName } }), {
...
;[0, 1].forEach((exitCode) => {
  it.live(`npm isolates and removes its install cache after exit ${exitCode}`, () =>
    Effect.gen(function* () {
      const test = yield* fixture((command) => {
        expect(command.command).toBe("npm")
        expect(existsSync(command.args[3])).toBe(true)
        return { exitCode, stderr: Buffer.from("npm install failed") }
      })
      const result = yield* test.updater.upgrade("npm", "v2.3.4-beta.1").pipe(Effect.flip, Effect.option)
      const cache = test.commands[0]?.[4]
      expect(cache).toStartWith(path.join(test.global.cache, "update-"))
      expect(test.commands).toEqual([["npm", "install", "--global", "--cache", cache, "@opencode-ai/cli@2.3.4-beta.1"]])
      expect(yield* test.fs.readDirectory(test.global.cache)).toEqual([])
      expect(result._tag).toBe(exitCode === 0 ? "None" : "Some")
      if (result._tag === "Some") expect(result.value.message).toBe("npm install failed")
    }),
  )
...
test("Node distribution honors the compile-time CLI name", async () => {
  const child = Bun.spawn(
    ...
    ],
    {
      ...
      stdout: "ignore",
      stderr: "pipe",
      // Compile-time defines must not reuse transpiler output from the parent test.
  ...
  it.live("Node distribution resolves the published npm package", () =>
    Effect.gen(function* () {
      ...
      expect(test.commands).toEqual([
        ...
        ["bun", "pm", "ls", "-g"],
        ["yarn", "global", "list"],
        ["npm", "install", "--global", "--cache", cache, "opencode-node@2.3.4"],
...
it.live("npm uses endpoint package metadata to migrate artifact packages", () =>
  Effect.gen(function* () {
    const test = yield* fixture(() => ({}), "@opencode-ai/cli", "@opencode-ai/cli-private")
    yield* test.updater.upgrade("npm", "2.3.4")
    const cache = test.commands[0]?.[5]
    expect(cache).toStartWith(path.join(test.global.cache, "update-"))
    expect(test.commands).toEqual([
      [
