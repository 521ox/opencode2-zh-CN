...
export function decodePolicy(text: string): Policy | undefined {
  ...
  if ("update" in input) {
    if (input.update === "disable" || input.update === "notify") return input.update
    if (input.update === "auto") return "notify"
    return
  }
  if (!("autoupdate" in input)) return
  if (input.autoupdate === false) return "disable"
  if (input.autoupdate === "notify" || input.autoupdate === true) return "notify"
}
...
const make = Effect.gen(function* () {
  ...
  const installedPackage = yield* Effect.gen(function* () {
    ...
    if (!("bin" in input) || typeof input.bin !== "object" || input.bin === null) return
    const binaries = Object.values(input.bin)
    if (binaries.some((bin) => typeof bin === "string" && path.resolve(directory, bin) === executable)) return input.name
  ...
  const release = Effect.fnUntraced(function* () {
    ...
    if (!("metadata" in data) || typeof data.metadata !== "object" || data.metadata === null) {
      return yield* Effect.fail(new Error("Update information did not include a package"))
    }
    if (!("package" in data.metadata) || typeof data.metadata.package !== "string")
      return yield* Effect.fail(new Error("Update information did not include a package"))
  ...
  const inspect = Effect.fnUntraced(function* () {
    ...
    yield* Effect.logInfo("update check", { current: OPENCODE_VERSION, latest: version })
    if (action(OPENCODE_VERSION, version, policy) === "none") {
