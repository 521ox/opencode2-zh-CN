...
const latest = { version: "0.0.0-beta-17498", metadata: { package: "@opencode-ai/cli" } }
...
await Effect.runPromise(
  Effect.gen(function* () {
    ...
    const executable = path.join(directory, "package", "bin", "opencode2")
    yield* fs.makeDirectory(path.dirname(executable), { recursive: true })
    yield* fs.writeFileString(
      path.join(directory, "package", "package.json"),
      JSON.stringify({ name: "@opencode-ai/cli", bin: { opencode2: "bin/opencode2" } }),
    )
    ...
    yield* Effect.gen(function* () {
      ...
      yield* updater.apply(latest.version)
