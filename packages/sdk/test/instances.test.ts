...
test("Promise instance plugin ID collisions reject admission and reconstruct on retry", async () => {
    ...
    plugins: [
      {
        id: "account-prompts",
        setup() {
          const activation = `host:${++hostGeneration}`
          setups.push(activation)
          return () => {
            cleanups.push(activation)
          }
  ...
  expect(configured).toEqual(["alpha"])
  // Session creation keeps a healthy default Location generation. The colliding selected generation
  // activates the same host plugin independently, then tears only that failed generation down.
  expect(setups).toEqual(["host:1", "host:2"])
  expect(cleanups).toEqual(["host:2"])
  ...
  expect(configured).toEqual(["alpha", "alpha"])
  // Retry constructs a third generation: host plugin first, then the renamed instance plugin.
  expect(setups).toEqual(["host:1", "host:2", "host:3", "alpha"])
  expect(cleanups).toEqual(["host:2"])
  await opencode.close()
  expect(cleanups.toSorted()).toEqual(["host:1", "host:2", "host:3"])
}, 20_000)
