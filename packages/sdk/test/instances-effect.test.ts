...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const llm = yield* TestLLM.Service.pipe(Effect.provide(TestLLM.layer()))
      ...
      const embed: OpenCode.EmbedOptions = {
        overrides: [
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const opencode = yield* Layer.buildWithScope(layer, hostScope).pipe(Effect.map(Context.get(OpenCode.Service)))
      ...
      // Failed construction releases its instance resources without closing the host.
