...
export interface InstanceOptions<R = never> extends Omit<SdkInstances.Options, "configure"> {
  readonly configure: (key: string) => Effect.Effect<SdkInstances.Configuration, unknown, R | Scope.Scope>
}
...
export interface CreateOptions<R = never> extends Omit<ServerOptions, "hostname" | "port" | "password"> {
  ...
  readonly instances?: InstanceOptions<R>
}
...
export const create = Effect.fn("EmbeddedHost.create")(function* <R = never>(
  ...
  const selector = instances
    ? {
        key: instances.key,
        // The instance Scope must own configured resources, so its ambient services override the captured build Scope.
        configure: (key: string) =>
          instances
            .configure(key)
            .pipe(
              Effect.updateContext((ambient: Context.Context<Scope.Scope>) => Context.merge(services, ambient)),
            ),
      }
    : undefined
