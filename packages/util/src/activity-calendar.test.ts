...
describe("activity calendar", () => {
  ...
  test("localizes month labels without changing calendar boundaries or intensity", () => {
    const english = activityCalendar(input)
    const explicitEnglish = activityCalendar({ ...input, locale: "en-US" })
    const chinese = activityCalendar({ ...input, locale: "zh" })

    expect(explicitEnglish).toEqual(english)
    expect(chinese.months).toEqual([{ week: 0, label: "1月", span: 2 }])
    expect(chinese.weeks).toEqual(english.weeks)
    expect(chinese.months.map(({ week, span }) => ({ week, span }))).toEqual(
      english.months.map(({ week, span }) => ({ week, span })),
    )
  })
