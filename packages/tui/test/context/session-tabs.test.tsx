...
async function renderSessionTabs(
  ...
  const calls = createFetch(async (url, request) => {
    ...
    if (url.pathname === "/api/session/active")
      return json({ data: Object.fromEntries((options?.active ?? []).map((id) => [id, { type: "running" }])) })
...
test("preview navigation loads services only for the active location", async () => {
  const other = `${directory}/other-worktree`
  const setup = await renderSessionTabs("first", {
    home: true,
    persisted: ["first", "second"],
    sessionDirectories: { second: other, preview: other },
  })

  try {
    await wait(() => setup.sessions.includes("first") && setup.sessions.includes("second"))
    ...
    await wait(() => setup.tabs.isPreview("preview") && setup.sessions.includes("preview"))
    await Bun.sleep(350)

    expect(setup.locations).not.toContain(other)
    expect(setup.vcsLocations).not.toContain(other)
    expect(setup.requestPaths).toContain("/api/session/first/permission")
    expect(setup.requestPaths).toContain("/api/session/first/form")
    expect(setup.requestPaths).not.toContain("/api/session/second/permission")
    expect(setup.requestPaths).not.toContain("/api/session/second/form")
    expect(setup.requestPaths).not.toContain("/api/session/preview/permission")
    expect(setup.requestPaths).not.toContain("/api/session/preview/form")
  } finally {
    await setup.destroy()
  }
})
...
test("does not load foreign location metadata when a background session moves", async () => {
  const destination = `${directory}/moved-worktree`
  const setup = await renderSessionTabs("first", { persisted: ["first", "second"] })

  try {
    await wait(() => setup.data.session.get("second") !== undefined)
    setup.emit({
      id: "evt_moved",
      created: 1,
      type: "session.moved",
    ...
    await Bun.sleep(20)
    expect(setup.locations).not.toContain(destination)
    expect(setup.vcsLocations).not.toContain(destination)
  } finally {
    await setup.destroy()
  }
})
...
test("retains a routed session family before registering its tab", async () => {
  const setup = await renderSessionTabs("first")

  try {
    await wait(() => setup.tabs.current() === "first" && setup.data.session.get("first") !== undefined)
    const retain = spyOn(setup.data.session, "retain")
    setup.route.navigate({ type: "session", sessionID: "second" })
    await wait(() => setup.tabs.current() === "second" && setup.tabs.tabs().some((tab) => tab.sessionID === "second"))
    expect(retain).toHaveBeenCalledWith("second")
  } finally {
    await setup.destroy()
  }
})

test("requests deferred data release only after closing the final running family tab", async () => {
  const sessionID = "ses_memory_running"
  const setup = await renderSessionTabs(sessionID, { active: [sessionID] })

  try {
    await wait(
      () =>
        setup.tabs.tabs().some((tab) => tab.sessionID === sessionID) && setup.data.session.get(sessionID) !== undefined,
      2_000,
      "initial tab and session data",
    )
    await wait(() => setup.data.session.status(sessionID) === "running", 2_000, "session running")
    const release = spyOn(setup.data.session, "releaseWhenIdle")

    setup.tabs.close(sessionID)
    await wait(() => setup.tabs.tabs().length === 0, 2_000, "tab closed")
...
test("reopening a closed running family retains it before terminal cleanup", async () => {
  const sessionID = "ses_memory_reopen"
  const setup = await renderSessionTabs(sessionID, { active: [sessionID] })
  ...
  try {
    ...
    setup.tabs.reopen()
    await wait(
      () => setup.route.data.type === "session" && setup.route.data.sessionID === sessionID,
      2_000,
      "route reopened",
    )
    expect(retain).toHaveBeenCalledWith(sessionID)
    expect(setup.data.session.get(sessionID)).toBeDefined()
    await wait(() => setup.tabs.tabs().some((tab) => tab.sessionID === sessionID), 2_000, "tab reopened")
    expect(setup.tabs.tabs().map((tab) => tab.sessionID)).toEqual([sessionID])
  } finally {
    await setup.destroy()
  }
})
