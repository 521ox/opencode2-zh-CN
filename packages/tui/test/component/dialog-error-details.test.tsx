...
for (const width of [40, 100]) {
  test(`error details stay compact at ${width} columns and investigate only prepares a draft`, async () => {
    ...
    try {
      app.renderer.start()
      await app.waitForFrame((frame) => frame.includes("err_a1b2c3d4"))
      const frame = app.captureCharFrame()
      expect(frame).toContain("×")
      expect(frame).toContain("broken/index.ts")
      expect(frame).not.toContain("very-long-temporary-directory")
