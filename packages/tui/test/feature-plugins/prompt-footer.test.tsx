...
test("prompt footer can hide localized details", async () => {
  const color = RGBA.fromInts(200, 200, 200)
  const context = {
    location: { directory: "/workspace" },
  ...
  try {
    await app.renderOnce()
    expect(app.captureCharFrame()).toContain("1.0K (10%) · US$1.00")
    expect(app.captureCharFrame()).toContain("ctrl+p 命令")

    setShowDetails(false)
    await app.renderOnce()
    const frame = app.captureCharFrame()
    expect(frame).not.toContain("1.0K (10%)")
    expect(frame).not.toContain("US$1.00")
    expect(frame).not.toContain("ctrl+p 命令")

    setSessionID(undefined)
    await app.renderOnce()
    expect(app.captureCharFrame()).not.toContain("shift+tab 智能体")
  } finally {
    app.renderer.destroy()
  }
})
