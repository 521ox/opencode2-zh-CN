  ...
  try {
    ...
    const task = Effect.runPromise(
      run({
        ...
        server: { endpoint: { url: server.url.toString() } },
        config: {
          get: async () => ({ animations: false, locale: "en" as const, keybinds: { "mcp.list": "f6" } }),
          update: async () => ({}),
        },
        packages: { prepare: async () => ({ directory: "" }) },
        args: {},
...
test("does not render completed compaction facts in the transcript", async () => {
  ...
  const messages = [
    ...
    })),
    {
      ...
      status: "completed",
      reason: "manual",
      summary: "COMPACTION SUMMARY MUST NOT RENDER",
      recent: "",
      time: { created: 8 },
    },
  ]
  ...
  try {
    ...
    const mounted = await setup.waitForFrame((frame) => frame.includes("Final visible message"))
    expect(mounted).not.toContain("COMPACTION SUMMARY MUST NOT RENDER")
    expect(mounted).not.toContain("Compaction")
  ...
  try {
    ...
    const task = Effect.runPromise(
      run({
        app: { name: "test", version: "test", channel: "test" },
        server: { endpoint: { url: server.url.toString() } },
        config: { get: async () => ({ animations: false, locale: "en" as const }), update: async () => ({}) },
        packages: { prepare: async () => ({ directory: "" }) },
        args: {},
        terminalHandoff: async () => ({ renderer: setup.renderer, mode: "dark", complete: ready.resolve }),
        log: () => {},
...
test.each([
  ...
  let inventory: PluginInfo[] = [initial ? failed : active]
  let requests = 0
  await using setup = await createAppFixture({
    width,
    state: state.path,
    config: { animations: false, locale: "en" },
    fetch: (url) => {
  ...
  setup.mockInput.pressEnter()
  await setup.waitForFrame((frame) => frame.includes("transform failed") && frame.includes("err_fixture"))
