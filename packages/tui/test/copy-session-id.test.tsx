...
test.each([
  { mode: "success", locale: "zh", title: "复制会话 ID", group: "会话", message: "已复制到剪贴板" },
  {
    mode: "failure",
    locale: "en",
  ...
  { mode: "home", locale: "zh", title: "复制会话 ID", group: "会话", message: "" },
] as const)("Copy session ID from Ctrl+P ($mode, $locale)", async ({ mode, locale, title, group, message }) => {
  ...
  const task = Effect.runPromise(
    run({
      ...
      config: { get: async () => ({ animations: false, locale }), update: async () => ({}) },
  ...
  try {
    ...
    const commandRow = new RegExp(`${title}\\s+${group}`)
    if (mode === "home") {
      await setup.waitForVisualIdle()
      expect(setup.captureCharFrame()).not.toMatch(commandRow)
      expect(copy).not.toHaveBeenCalled()
      return
    }
    await setup.waitForFrame((frame) => commandRow.test(frame))
    setup.mockInput.pressEnter()
    const frame = await setup.waitForFrame((frame) => frame.includes(message))
    expect(copy).toHaveBeenCalledTimes(1)
    expect(copy.mock.calls[0]?.[0]).toBe(sessionID)
    expect(frame).not.toMatch(commandRow)
    await setup.waitFor(
      () =>
