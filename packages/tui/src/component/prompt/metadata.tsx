...
export function PromptMetadataRow(props: {
  ...
  const layout = createMemo(() => {
    if (props.mode === "shell") return { agent: props.agent ?? "", autoLabel: props.autoLabel, model: "" }
    ...
    <box flexDirection="row" gap={1} flexGrow={1} flexShrink={1} minWidth={0}>
      ...
      >
        ...
        <Show when={props.mode === "normal" && layout().auto}>
          <text fg={fade(theme.text.subdued, props.agentAlpha)}>{props.autoLabel}</text>
        </Show>
...
function promptMetadataLayout(input: {
  ...
  const candidates: Layout[] = [
    { agent, auto: input.auto, autoLabel: input.autoLabel, model: input.model, provider, variant: input.variant },
    { agent, autoLabel: input.autoLabel, model: input.model, provider, variant: input.variant },
    {
      agent,
    ...
    { agent, autoLabel: input.autoLabel, model: input.model, variant: input.variant },
  ]
