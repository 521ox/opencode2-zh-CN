...
export function StatsPoster(props: { stats: SessionStatsInfo }) {
  ...
  const metrics = createMemo(() => statsMetrics(props.stats, i18n.t))
  const number = () => statsNumber(metrics()[0].value, i18n.locale())
  const dates = createMemo(() =>
    new Intl.DateTimeFormat(i18n.locale(), { month: "short", year: "numeric" }).formatRange(
      new Date(props.stats.range.from),
      new Date(Math.max(props.stats.range.from, props.stats.range.to - 1)),
    ),
  )
  const weekdays = createMemo(() => {
    const formatter = new Intl.DateTimeFormat(i18n.locale(), { weekday: "narrow", timeZone: "UTC" })
    return Array.from({ length: 7 }, (_, day) => formatter.format(Date.UTC(2026, 0, 5 + day)))
  })
  ...
  return (
    <box width={width()} flexDirection="column" alignItems="center" flexShrink={0} gap={compact() ? 1 : 2}>
      ...
      <box alignItems="center" gap={1}>
        ...
        <text fg={theme.text.subdued}>{i18n.t("feature.stats.tokens")}</text>
      </box>
      <box alignItems="center">
        ...
        <Show when={calendar().clipped}>
          <text fg={theme.text.subdued}>
            {i18n.t("feature.stats.range.clipped", { count: calendar().weeks.length })}
          </text>
        </Show>
      </box>
        ...
        <For each={metrics().slice(1)}>
          {(metric) => (
            <box alignItems="center">
              <text fg={theme.text.default} attributes={TextAttributes.BOLD}>
                {metric.unit
                  ? i18n.t(metric.unit, { count: statsNumber(metric.value, i18n.locale()) })
                  : statsNumber(metric.value, i18n.locale())}
              </text>
...
function StatsPage(props: { context: Plugin.Context; onClose: () => void }) {
  ...
  props.context.keymap.layer(() => ({
    commands: [{ bind: "escape", title: i18n.t("feature.stats.command.back"), run: props.onClose }],
  }))
    ...
    <box width="100%" height="100%" backgroundColor={theme.background.default}>
      ...
      >
        <Show
          when={!result.error}
          fallback={
            <text fg={theme.text.feedback.error.default}>{i18n.t("feature.stats.error")}</text>
          }
        >
          <Show when={result()} fallback={<text fg={theme.text.subdued}>{i18n.t("feature.stats.loading")}</text>}>
...
function StatsCommands(props: { context: Plugin.Context; onOpen: () => void }) {
  const i18n = useI18n()
  props.context.keymap.layer(() => ({
    mode: "global",
    commands: [
      {
        id: "stats.open",
        title: i18n.t("feature.stats.command.open"),
        group: i18n.t("feature.stats.group"),
        slash: { name: "stats" },
        palette: true,
        run: props.onOpen,
      },
