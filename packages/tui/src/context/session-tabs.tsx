...
export const { use: useSessionTabs, provider: SessionTabsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    createEffect(() => {
      ...
      if (signature === "") return
      const activeLocationKey = locationKey(location.ref ?? data.location.default())
      ...
      void (async () => {
        ...
        const activeLocation = locations.get(activeLocationKey)
        if (activeLocation)
          await Promise.allSettled([data.location.syncInfo(activeLocation), data.location.vcs.sync(activeLocation)])
      })()
      const timer = setTimeout(async () => {
        ...
        for (const sessionID of sessions) {
          if (stale) return
          const sessionLocation = data.session.get(sessionID)?.location
          const active = sessionLocation !== undefined && locationKey(sessionLocation) === activeLocationKey
          await Promise.allSettled([
            data.session.message.sync(sessionID),
            data.session.pending.sync(sessionID),
            ...(active ? [data.session.permission.sync(sessionID), data.session.form.sync(sessionID)] : []),
          ])
        }
      }, TAB_PREFETCH_DELAY)
      onCleanup(() => {
    ...
    onCleanup(
      event.on("session.moved", (evt) => {
        ...
        if (!enabled() || !state().tabs.some((tab) => tab.sessionID === sessionID)) return
        if (
          sessionID !== current() &&
          locationKey(evt.data.location) !== locationKey(location.ref ?? data.location.default())
        )
          return
    ...
    function remove(sessionID: string, navigate: boolean) {
      ...
      if (!closed.tabs.some((tab) => root(tab.sessionID) === target)) data.session.releaseWhenIdle(target)
    ...
    return {
      ...
      reopen() {
        ...
        data.session.retain(result.sessionID)
