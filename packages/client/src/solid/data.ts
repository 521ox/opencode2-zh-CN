...
function createSync() {
  ...
  type Completion = void | boolean
  const state = new Map<string, true | Pending>()
  const start = (key: string, load: () => Promise<Completion>, wait?: Promise<void>) => {
    ...
    entry.promise = (wait ? wait.catch(() => undefined).then(load) : load())
      .then((completion) => {
        ...
        // A fenced response consumed its request but must not recreate this key as complete.
        if (completion === false || entry.invalidated) {
          state.delete(key)
          return
        }
        state.set(key, true)
  ...
  return {
    run(key: string, load: () => Promise<Completion>) {
...
export function createData(config: CreateDataInput) {
  ...
  function settleForm(input: FormCancelInput, ref: LocationRef | undefined, request: Promise<void>) {
    return request
      ...
      .then(() => {
        ...
        if (flushReleasedFamily(input.sessionID)) return
  ...
  type FamilyOwnership = { root: string; owner: object }
  const releasedFamilies = new Set<string>()
  const retainedFamilies = new Set<string>()
  const familyOwners = new Map<string, object>()

  function captureFamilyOwner(sessionID: string): FamilyOwnership {
    const root = resolveRoot(sessionID)
    let owner = familyOwners.get(root)
    if (!owner) {
      owner = {}
      familyOwners.set(root, owner)
    }
    return { root, owner }
  }

  function ownsFamily(ownership: FamilyOwnership) {
    return familyOwners.get(ownership.root) === ownership.owner
  }

  function captureActiveSnapshot() {
    const owners = new Map<string, object>()
    const ownerless = new Set<string>()
    for (const [root, owner] of familyOwners) {
      for (const sessionID of [root, ...(store.session.family[root] ?? [])]) owners.set(sessionID, owner)
    }
    for (const sessionID of Object.keys(store.session.active)) {
      const owner = familyOwners.get(resolveRoot(sessionID))
      if (owner) owners.set(sessionID, owner)
      else ownerless.add(sessionID)
    }
    return { owners, ownerless }
  }

  function ownsActiveSession(sessionID: string, owner: object) {
    return familyOwners.get(resolveRoot(sessionID)) === owner
  }

  function isOwnerlessActiveSession(sessionID: string) {
    return !familyOwners.has(resolveRoot(sessionID))
  }

  function moveFamilyOwner(sessionID: string, rootID: string) {
    if (sessionID === rootID) return
    const owner = familyOwners.get(sessionID)
    const released = releasedFamilies.delete(sessionID)
    const retained = retainedFamilies.delete(sessionID)
    const targetRetained = retainedFamilies.has(rootID)
    if (retained || targetRetained) {
      retainedFamilies.add(rootID)
      releasedFamilies.delete(rootID)
    } else if (released) {
      releasedFamilies.add(rootID)
    }
    if (owner && !familyOwners.has(rootID)) familyOwners.set(rootID, owner)
    familyOwners.delete(sessionID)
  }
  ...
  function registerSession(sessionID: string) {
    ...
    moveFamilyOwner(sessionID, rootID)
    setStore(
      "session",
      "family",
      produce((draft) => {
  ...
  function invalidateSessionSync(sessionID: string) {
  ...
  function removeSession(sessionID: string) {
    for (const [inboxID, owner] of outbox) {
      if (owner === sessionID) outbox.delete(inboxID)
    }
    sessionOutbox.delete(sessionID)
    creating.delete(sessionID)
    sending.delete(sessionID)
    messageIndex.delete(sessionID)
    invalidateSessionSync(sessionID)
    setStore(
      "session",
      produce((draft) => {
    ...
    if (!store.session.info[sessionID] && !store.session.family[sessionID]) retainedFamilies.delete(sessionID)
  }

  function familyMembers(sessionID: string) {
    const root = resolveRoot(sessionID)
    return { root, members: [...(store.session.family[root] ?? [root])] }
  }

  function familyBusy(sessionID: string) {
    const { members } = familyMembers(sessionID)
    return members.some(
      (id) =>
        store.session.active[id] === "running" ||
        (store.session.pending[id]?.length ?? 0) > 0 ||
        (store.session.permission[id]?.length ?? 0) > 0 ||
        (store.session.form[id]?.length ?? 0) > 0,
    )
  }

  function invalidateFamilyOwner(family: ReturnType<typeof familyMembers>) {
    familyOwners.delete(family.root)
    for (const id of family.members) familyOwners.delete(id)
  }

  function flushReleasedFamily(sessionID: string) {
    const family = familyMembers(sessionID)
    if (!releasedFamilies.has(family.root) || familyBusy(family.root)) return false
    // Invalidate the generation before cache/index removal so delayed loads cannot revive it.
    invalidateFamilyOwner(family)
    releasedFamilies.delete(family.root)
    retainedFamilies.delete(family.root)
    for (const id of family.members) removeSession(id)
    return true
  }

  function releaseFamilyWhenIdle(sessionID: string) {
    const root = resolveRoot(sessionID)
    if (!familyOwners.has(root)) familyOwners.set(root, {})
    retainedFamilies.delete(root)
    releasedFamilies.add(root)
    return flushReleasedFamily(root)
  }

  function retainFamily(sessionID: string) {
    const root = resolveRoot(sessionID)
    retainedFamilies.add(root)
    releasedFamilies.delete(root)
    if (!familyOwners.has(root)) familyOwners.set(root, {})
  }
  ...
  function handleEvent(event: OpenCodeEvent) {
    switch (event.type) {
          ...
          api()
            ...
            .then((active) => {
              setStore(
                ...
                "active",
                produce((draft) => {
                  for (const [sessionID, owner] of activeSnapshot.owners) {
                    if (!ownsActiveSession(sessionID, owner)) continue
                    if (Object.hasOwn(active, sessionID)) draft[sessionID] = "running"
                    else delete draft[sessionID]
                  }
                  for (const sessionID of activeSnapshot.ownerless) {
                    if (!isOwnerlessActiveSession(sessionID)) continue
                    if (Object.hasOwn(active, sessionID)) draft[sessionID] = "running"
                    else delete draft[sessionID]
                  }
                  for (const sessionID of Object.keys(active)) {
                    if (activeSnapshot.owners.has(sessionID) || activeSnapshot.ownerless.has(sessionID)) continue
                    if (isOwnerlessActiveSession(sessionID)) draft[sessionID] = "running"
                  }
                }),
              )
            })
      ...
      case "session.deleted": {
        const family = familyMembers(event.data.sessionID)
        const rootExists = family.root !== event.data.sessionID && store.session.info[family.root] !== undefined
        const released = releasedFamilies.has(family.root)
        invalidateFamilyOwner(family)
        for (const id of family.members) invalidateSessionSync(id)
        removeSession(event.data.sessionID)
        flushReleasedFamily(family.root)
        if (!released && rootExists) familyOwners.set(family.root, {})
        return
      }
      ...
      case "session.model.selected":
        ...
        const modelMessageOwner = captureFamilyOwner(event.data.sessionID)
        refresh(() =>
          api()
            .session.message({ sessionID: event.data.sessionID, messageID: messageIDFromEvent(event.id) })
            .then((item) => {
              if (!ownsFamily(modelMessageOwner)) return
      ...
      case "session.renamed":
        ...
        const renamedOwner = captureFamilyOwner(event.data.sessionID)
        refresh(() =>
          result.session.sync(event.data.sessionID).then(() => {
            if (!ownsFamily(renamedOwner)) return
            if (store.session.info[event.data.sessionID])
      ...
      case "session.inbox.delivered": {
        ...
        flushReleasedFamily(event.data.sessionID)
        return
      }
      ...
      case "session.execution.interrupted":
        ...
        if (flushReleasedFamily(event.data.sessionID)) return
  ...
  // Cached per-location catalogs publish into the existing Location store so sibling
  // resources retain their reactive references when one catalog refreshes.
  ...
  const result = {
    ...
    session: {
      retain: retainFamily,
      releaseWhenIdle: releaseFamilyWhenIdle,
      list() {
        return sessions()
      },
      ...
      evict(sessionID: string) {
        const family = familyMembers(sessionID)
        // Rotate the existing owner before clearing so in-flight reads cannot refill evicted caches.
        familyOwners.set(family.root, {})
        batch(() => {
          for (const id of new Set([family.root, sessionID, ...family.members])) evictSession(id)
        })
      },
      ...
      pending: {
        ...
        sync(sessionID: string) {
          ...
          return sync.run(`session.pending:${sessionID}`, async () => {
            ...
            if (flushReleasedFamily(sessionID)) return false
            return true
          })
        },
      ...
      create(input: {
        ...
        const ownership = captureFamilyOwner(id)
        ...
        const request = Promise.resolve()
          ...
          .then((info) => {
            ...
            if (!ownsFamily(ownership)) return info
      ...
      prompt(input: SessionPromptInput & { gate?: Promise<unknown> }) {
        ...
        if (fresh) {
          outbox.set(id, request.sessionID)
          admitLocal({
            id,
        ...
        return send.catch((error) => {
          ...
          if (fresh && outbox.delete(id)) {
            ...
            flushReleasedFamily(request.sessionID)
          }
          throw error
        })
      },
      ...
      message: {
        ...
        async loadMore(sessionID: string) {
          ...
          try {
            const response = await api().message.list({ sessionID, limit: messagePageLimit, cursor })
            if (!ownsFamily(ownership)) return
            ...
            if (ownsFamily(ownership)) setStore("session", "messageLoading", sessionID, false)
          }
        },
        invalidate(sessionID: string) {
          sync.invalidate(`session.message:${sessionID}`)
      ...
      permission: {
        ...
        sync(sessionID: string) {
          ...
          return sync.run(`session.permission:${sessionID}`, async () => {
            const permission = await api().permission.list({ sessionID })
            if (!ownsFamily(ownership)) return false
            setStore("session", "permission", sessionID, permission)
            if (flushReleasedFamily(sessionID)) return false
            return true
          })
        },
        ...
        async reply(input: PermissionReplyInput) {
          ...
          flushReleasedFamily(input.sessionID)
        },
      },
      form: {
        ...
        sync(sessionID: string, ref?: LocationRef) {
          ...
          const ownership = sessionID === "global" ? undefined : captureFamilyOwner(sessionID)
          return sync.run(key, async () => {
            ...
            const form = await api().form.list({ sessionID })
            if (!ownership || !ownsFamily(ownership)) return false
            setStore("session", "form", sessionID, form)
            if (flushReleasedFamily(sessionID)) return false
            return true
          })
        },
