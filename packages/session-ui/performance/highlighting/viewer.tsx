...
async function mount(revision: number) {
  ...
  const pool = getWorkerPool(large ? "unified" : "split")!
  let firstReady = 0
  let rendered = 0
...
export const highlighting = {
  mount,
  configure(options: Partial<WorkerRenderingOptions>) { return getWorkerPool(large ? "unified" : "split")!.setRenderOptions(options) },
