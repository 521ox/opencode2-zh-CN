...
type IsRenderable = (content: Content, showReasoning: boolean, detail?: TimelineDetail, partID?: string) => boolean
...
export function createReactiveTimelineProjection(input: {
  ...
  // Stable part IDs keep accessors alive when stores replace content proxies.
  const textContent = createMemo(
    () =>
      new Map(
        input.sessionMessages().flatMap((message) =>
          message.type === "assistant"
            ? Timeline.contentEntries(message)
                .flatMap((entry) =>
                  entry.content.type === "tool" ? [] : [[entry.id, entry.content] as const],
                )
            : [],
        ),
      ),
  ...
  const textParts = mapArray(
    () => [...textContent().keys()],
    (partID) => [partID, createMemo(() => !!textContent().get(partID)?.text.trim())] as const,
  )
  const textVisible = createMemo(() => new Map<string, Accessor<boolean>>(textParts()))
...
export namespace Timeline {
  export function constructSessionMessageRows(
    ...
    isRenderable: IsRenderable = renderable,
