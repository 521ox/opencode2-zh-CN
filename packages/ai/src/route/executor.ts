  ...
  Effect.gen(function* () {
    ...
    const executeOnce = (
      ...
      Effect.gen(function* () {
        ...
        const handler: HttpHandler = dispatch
          ? (input) =>
              dispatch(input).pipe(
        ...
        const response = yield* middleware(request, handler).pipe(
          Effect.mapError((error) =>
            dispatch && error instanceof AIError ? error : httpError({ error, request, operation: "request" }),
          ),
        )
