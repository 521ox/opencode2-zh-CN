...
export interface ProviderAdapter {
  ...
  /** Replays validated provider-owned input items without serializing them through local text. */
...
const lowerMessages = Effect.fn("OpenResponses.lowerMessages")(function* (
  ...
  for (const message of request.messages) {
    ...
    if (message.role === "user") {
      const remote = adapter.lowerUserMessage?.({ message, providerMetadataKey })
      if (remote) {
        input.push(...remote)
        continue
      }
      input.push({
        role: "user",
        content: yield* Effect.forEach(message.content, (part) => lowerUserContent(part, request, adapter)),
        type: metadata?.type,
        id: metadata?.itemId,
    ...
    if (message.role === "assistant") {
      ...
      const flushText = () => {
        ...
        >((groups, part) => {
          ...
          const partPhase = ProviderShared.isRecord(partMetadata) ? messagePhase(partMetadata.phase) : undefined
...
export const allowedToolChoice = (request: LLMRequest) => {
  ...
  if (request.tools.length === 0 || !allowed) return undefined
  return {
...
// Resolve wire item ids once so protocol extensions and the base parser share
// one item identity. Tool events and tombstones remain keyed by call_id.
...
const onOutputItemAdded = (state: ParserState, event: NormalizedEvent): StepResult => {
  ...
  if (item?.type === "message") {
    ...
    const completedMessages = new Set([
      ...
      ...[...state.messageItems].filter((id) => id !== itemID),
    ])
...
const onOutputItemDone = Effect.fn("OpenResponses.onOutputItemDone")(function* (state: ParserState, event: Event) {
  ...
  if (item.type === "compaction") {
    ...
    const events: LLMEvent[] = []
    const lifecycle = Lifecycle.stepStart(state.lifecycle, events)
    events.push(LLMEvent.compaction({ provider: state.provider, id: item.id, encrypted: item.encrypted_content }))
    return [
  ...
  if (item.type === "message" && item.id !== undefined) {
    ...
    if (!state.messageItems.has(item.id) && state.messageItems.size > 0)
    ...
    const content = parts.flatMap((part) => {
      if (!ProviderShared.isRecord(part)) return []
    ...
    const lifecycle =
      text && !state.lifecycle.text.has(item.id)
        ? Lifecycle.textDelta(Lifecycle.textStart(state.lifecycle, events, item.id, metadata), events, item.id, text)
        : state.lifecycle
    return [
      {
        ...state,
        lifecycle: Lifecycle.textEnd(lifecycle, events, item.id, metadata),
        completedMessages,
        messageItems,
        messagePhases,
      },
...
const onResponseFinish = Effect.fn("OpenResponses.onResponseFinish")(function* (state: ParserState, event: Event) {
  ...
  if (event.type === "response.completed") {
    for (const [index, input] of (event.response?.output ?? []).entries()) {
      const item = resolveItem(state, input, index)
      ...
      if (item.type === "function_call") {
        if (!item.call_id || !Object.values(current.tools).some((tool) => tool?.id === item.call_id)) continue
      } else if (item.type === "reasoning") {
        if (!current.reasoningItems[item.id]) continue
      } else if (item.type !== "compaction") {
        continue
      }
      const [next, emitted] = yield* onOutputItemDone(current, { type: "response.output_item.done", item })
      current = next
      events.push(...emitted)
    }
  }
...
export const step = (state: ParserState, event: NormalizedEvent) => {
  ...
  if (event.type === "response.reasoning.delta" || event.type === "response.reasoning_summary_text.delta") {
    ...
    if (state.reasoningItems[event.item_id] === undefined)
      return ProviderShared.eventError(state.id, `${event.type} arrived before reasoning item start`)
  ...
  if (
    ...
    if (event.item_id === undefined) return ProviderShared.eventError(state.id, `${event.type} is missing item_id`)
    if (state.reasoningItems[event.item_id] === undefined)
      return ProviderShared.eventError(state.id, `${event.type} ended before reasoning item start`)
  ...
  if (event.type === "response.output_item.added") {
    if (event.item && isReasoningItem(event.item) && state.reasoningItems[event.item.id] !== undefined)
      return ProviderShared.eventError(state.id, `${event.type} duplicated reasoning item ${event.item.id}`)
    return Effect.succeed(
      onOutputItemAdded(
