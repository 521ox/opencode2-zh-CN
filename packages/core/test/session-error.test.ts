...
describe("toSessionError", () => {
  test("maps every AI error reason to the open wire type", () => {
    ...
    expect(
      toSessionError(llm(new UnsupportedOperationError({ message: "unsupported", operation: "compact" }))),
    ).toEqual({ type: "provider.unsupported-operation", message: "unsupported" })
    expect(
      toSessionError(
        llm(
          new NoRouteError({
  ...
  test("retries only rate limits, provider-internal failures, and transport failures", () => {
    ...
    expect(ineligible.map(SessionRunnerRetry.isRetryable)).toEqual([false, false, false, false, false, false, false, false])
  })
