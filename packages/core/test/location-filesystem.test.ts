...
describe("FileSystem", () => {
  ...
  it.live("lists parents and siblings through the current location service", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          expect(parent.map((entry) => ({ path: entry.path, type: entry.type }))).toEqual(
            expect.arrayContaining([
              { path: RelativePath.make("." + path.sep), type: "directory" },
              { path: RelativePath.make(path.join("..", "sibling") + path.sep), type: "directory" },
            ]),
          )
          const sibling = yield* filesystem.list({ path: path.join(directory, "sibling") })
  ...
  it.live("allows listing through an external symlink without allowing file reads", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          const filesystem = yield* FileSystem.Service
          const entries = yield* filesystem.list({ path: "link" })
          expect(entries.map((entry) => ({ path: entry.path, type: entry.type }))).toEqual([
