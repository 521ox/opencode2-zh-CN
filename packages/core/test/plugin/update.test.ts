...
const it = testEffect(AppNodeBuilder.build(PluginUpdate.node, [[Npm.node, npm]]))
...
it.effect("publishes successful package updates", () =>
  Effect.gen(function* () {
    ...
    const changed = yield* updates
      .changes()
      .pipe(Stream.filter((change) => !change.updating), Stream.take(1), Stream.runHead, Effect.forkScoped({ startImmediately: true }))

    yield* updates.update("fixture")

    expect(Option.getOrUndefined(yield* Fiber.join(changed))).toEqual({
      target: "fixture",
      outdated: false,
      updating: false,
    })
