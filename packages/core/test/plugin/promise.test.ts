...
describe("fromPromise", () => {
  ...
  it.live("keeps Effect and Promise namespace registration consistent", () =>
    Effect.gen(function* () {
      const plugins = yield* Plugin.Service
      const registry = yield* Tool.Service
      const host = yield* PluginHost.make(plugins)
      yield* host.tool.transform((draft) => {
        draft.namespace({ name: "acme", description: "Effect namespace" })
        draft.add({
          name: "hello",
          description: "Hello",
          options: { namespace: "acme", codemode: false },
      ...
      yield* PluginPromise.fromPromise(
        define({
          ...
          setup: async (ctx) => {
            await ctx.tool.transform((draft) => {
              draft.namespace({ name: "acme", description: "Promise namespace" })
              draft.update("acme_hello", (tool) => {
                delete tool.options
              })
      ...
      expect(snapshot.codeModeCatalog?.tools).toEqual([
        {
          type: "namespace",
          name: "acme",
          description: "Promise namespace",
          tools: [expect.objectContaining({ type: "tool", name: "hello" })],
        },
      ])
      expect(original.definitions.map((tool) => tool.name)).toEqual(["acme_hello", "execute"])
      expect(
