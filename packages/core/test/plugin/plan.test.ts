...
const reminders = Effect.gen(function* () {
  const current = yield* run([agentSelected(plan, build), agentSelected(build, plan)])
  yield* settle(current.persisted, 2)
  return { enter: current.persisted[0]!, leave: current.persisted[1]! }
})
...
describe("plan plugin reminders", () => {
  ...
  it.effect("reconciles a missing enter reminder into the request without persisting", () =>
    Effect.gen(function* () {
  ...
  it.effect("reconciles a stale enter reminder with a leave reminder without persisting", () =>
    Effect.gen(function* () {
      const { enter } = yield* reminders
      const { persisted, contextHook } = yield* run()
