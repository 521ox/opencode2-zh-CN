...
import { XAI } from "@opencode-ai/ai/providers"
...
modelLimits.set("native-responses", { context: 100_000, output: 1_000 })
const xaiResponsesModel = XAI.configure().responses("grok-test")
modelLimits.set("grok-test", { context: 100_000, output: 20_000 })
...
const makeRunnerState = () => {
  ...
  const currentModel: LanguageModel = model
  ...
  return {
    ...
    compactAttempts: new Array<LLMRequest>(),
    compactFailure,
    modelResolutions: 0,
    blockTools: (count = 1) =>
  ...
  Effect.map(RunnerState, (state) => {
    ...
    const replacements: LayerNode.Replacements = [
      ...
      [
        ...
        Layer.effect(
          LLMClient.Service,
          Effect.map(TestLLM.Service, (service) =>
            LLMClient.Service.of({
              ...service.client,
              compact: (request, options) => {
                state.compactAttempts.push(request)
                return state.compactFailure
                  ? Effect.fail(state.compactFailure)
                  : service.client.compact!(request, options)
              },
            }),
          ),
        ),
...
const insertHistoryMessage = (
  ...
  Effect.gen(function* () {
    const { id: _id, type: _type, ...data } = Schema.encodeSync(SessionMessage.Info)(message)
    yield* db
      .insert(SessionMessageTable)
      .values({ id: message.id, session_id: sessionID, type: message.type, seq, data })
      .run()
      .pipe(Effect.orDie)
  })
...
const setup = Effect.gen(function* () {
  ...
  state.compactAttempts.length = 0
  state.compactFailure = undefined
  state.modelResolutions = 0
...
const remoteCheckpoint = (id: string) => ({ type: "compaction", id, encrypted_content: `opaque-${id}` })

const explicitCheckpoint = (id: string, encrypted = `opaque-${id}`) =>
  new CompactionResponse({
    replacement: [Message.assistant(CompactionPart.make({ provider: ProviderID.make("xai"), id, encrypted }))],
  })

const compact404 = () =>
  RequestExecutor.httpFailure({
    message: "Provider request failed with HTTP 404",
    url: "https://api.x.ai/v1/responses/compact",
    status: 404,
  })
...
const malformedCompaction = () =>
  new AIError({
    reason: new InvalidProviderOutputError({
      ...
      message: "Invalid compaction response",
    }),
  })

const remoteTrigger = (checkpoint: ReturnType<typeof remoteCheckpoint>) => [
...
const hasRemoteCheckpoint = (request: LLMRequest, checkpoint: ReturnType<typeof remoteCheckpoint>) =>
  request.messages.some((message) =>
    message.content.some((part) => {
      const metadata = part.type === "text" ? part.providerMetadata?.opencode : undefined
      const remote =
        typeof metadata === "object" && metadata !== null
          ? (metadata as { readonly remoteCompaction?: { readonly output?: readonly unknown[] } }).remoteCompaction
          : undefined
      return (
        remote?.output?.some(
          (item) =>
            item === checkpoint ||
            (typeof item === "object" && item !== null && "id" in item && item.id === checkpoint.id),
        ) === true
      )
    }),
  )
...
const retryScheduleQueue = (bus: Bus.Interface, id: Session.ID) =>
  Effect.gen(function* () {
    const scheduled = yield* Queue.unbounded<void>()
    yield* bus.subscribe(SessionEvent.RetryScheduled).pipe(
      Stream.filter((event) => event.data.sessionID === id),
...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("appends the root Session rules directory after context hooks and the GPT extension", () =>
    Effect.gen(function* () {
      const s = yield* setup
      const hooks = yield* PluginHooks.Service
      ...
      yield* hooks.register("session", "context", (event) =>
        Effect.sync(() => {
          event.system.push(SystemPart.make("Hook context"))
        }),
      )
      preserveRulesContext = true
      yield* TestLLM.push(TestLLM.text("Done", "text-rules-context"))

      yield* s.runPrompt("Use the Session rules context")
      ...
      expect(s.requests[0]?.system.map((part) => part.text)).toEqual([
        ...
        "Initial context",
        "Hook context",
        expect.stringContaining(SessionRulesLocation.Header),
      ])
      const text = s.requests[0]?.system.at(-1)?.text ?? ""
  ...
  it.effect("injects a missing Plan reminder without enqueueing another turn", () =>
    Effect.gen(function* () {
      const s = yield* setup
      const agents = yield* Agent.Service
      const hooks = yield* PluginHooks.Service
      ...
      ).pipe(Effect.provideService(Global.Service, Global.Service.of({ ...Global.make(), home: "/plan" })))
      yield* s.db
        .update(SessionTable)
        .set({ agent: Agent.ID.make("plan") })
        .where(eq(SessionTable.id, sessionID))
        .run()
        .pipe(Effect.orDie)

      yield* s.admit("Create a plan")
      yield* TestLLM.push(TestLLM.text("Plan response", "text-plan-response"))
      yield* s.session.resume(sessionID)

      expect(s.requests).toHaveLength(1)
      const user = s.requests[0]?.messages.flatMap((message) =>
        message.role === "user"
          ? message.content.flatMap((content) => (content.type === "text" ? [content.text] : []))
          : [],
      )
      expect(user?.[0]).toContain("You are in Plan mode")
      expect(user?.at(-1)).toBe("Create a plan")
      expect(
        yield* s.db
          .select()
      ...
      expect(yield* s.session.context(sessionID)).toMatchObject([
        { type: "user", text: "Create a plan" },
        { type: "assistant", content: [{ type: "text", text: "Plan response" }] },
      ])
    }),
  )
  ...
  scenario("runs steers before queued compaction and later queued input", function* (s) {
    ...
    expect(userTexts(s.requests[2]).at(-1)).toBe(SessionCompaction.buildPrompt(false))
  ...
  scenario("settles a private continuation from its promoted Inbox turn after runner post-step policy", function* (s) {
    ...
    const identity = SessionContinuation.identity({
      ...
      parentMessageID,
      parentToolCallID: "runner-private-continuation",
      prompt: "Return the exact continuation final",
    })
    const input: SessionContinuation.Request = {
      ...identity,
      parentSessionID: sessionID,
      parentMessageID,
      parentToolCallID: "runner-private-continuation",
      ...
      agent: "build",
      description: "Runner continuation",
    }
    yield* SessionInbox.admit(s.db, s.bus, {
      id: input.inboxID,
      sessionID,
      item: { type: "user", payload: { text: "Return the exact continuation final" }, delivery: "steer" },
      commit: () => continuations.admit(input).pipe(Effect.orDie),
    })
    yield* s.llm.push(TestLLM.text("exact runner continuation final", "runner-continuation-final"))
    yield* s.resume

    expect(yield* continuations.await(input.id)).toBe("exact runner continuation final")
    expect(yield* continuations.get(input.id)).toMatchObject({
      state: "completed",
      turn: { output: "exact runner continuation final", state: "completed" },
    })
    expect(yield* continuations.active(sessionID)).toBeUndefined()
  })

  scenario("uses one native trigger request for manual compaction", function* (s) {
    s.currentModel = nativeResponsesModel
    const hooks = yield* PluginHooks.Service
    yield* hooks.register("session", "context", (event) =>
      Effect.sync(() => event.system.push(SystemPart.make("ordinary native trigger prefix"))),
    )
    preserveRulesContext = true
    yield* s.llm.push(TestLLM.text("Earlier native answer", "text-manual-native-history"))
    yield* s.runPrompt("Earlier native question")

    const ordinarySystem = JSON.stringify(s.requests[0]?.system)
    expect(ordinarySystem).toContain(SessionRulesLocation.Header)
    s.requests.length = 0
    const checkpoint = remoteCheckpoint("cmp_manual")
    yield* s.llm.push(remoteTrigger(checkpoint))
    const compaction = yield* s.session.compact({ sessionID })
    yield* s.resume

    expect(s.requests).toHaveLength(1)
    expect(userTexts(s.requests[0]!)).toContain("Earlier native question")
    expect(s.requests[0]?.tools).toEqual([])
    expect(s.requests[0]?.toolChoice).toBeUndefined()
    expect(s.requests[0]?.providerOptions?.compactThreshold).toBeUndefined()
    expect(JSON.stringify(s.requests[0]?.system)).not.toContain(SessionRulesLocation.Header)
    expect(s.requests[0]?.system.map((part) => part.text)).toContain("ordinary native trigger prefix")
    expect((yield* s.messages).find((message) => message.id === compaction.id)).toMatchObject({
      type: "compaction",
      status: "completed",
      reason: "manual",
  ...
  scenario("keeps xAI generation below the prompt ceiling on the ordinary request path", function* (s) {
    s.currentModel = xaiResponsesModel
    yield* s.llm.push(TestLLM.text("Below ceiling", "text-xai-below"))

    const input = yield* s.runPrompt("A small xAI request")

    expect(s.compactAttempts).toHaveLength(0)
    expect(s.modelResolutions).toBe(1)
    expect(s.requests).toHaveLength(1)
    expect(s.requests[0]?.model).toBe(xaiResponsesModel)
    expect(userTexts(s.requests[0])).toContain("A small xAI request")
    expect(s.requests[0]?.providerOptions?.compactThreshold).toBeUndefined()
    expect(yield* SessionInbox.find(s.db, input.id)).toBeUndefined()
  })

  scenario("compacts xAI once before generation and preserves the opaque checkpoint", function* (s) {
    yield* s.llm.push(TestLLM.textWithUsage("Earlier answer", "text-xai-earlier", 79_950))
    yield* s.runPrompt("Earlier xAI context ".repeat(180))
    s.currentModel = xaiResponsesModel
    s.requests.length = 0
    s.modelResolutions = 0
    const encrypted = "opaque-xai-\u0000-byte-stable"
    const pending = "Pending xAI turn ".repeat(20)
    yield* s.llm.push(explicitCheckpoint("cmp_xai_auto", encrypted), TestLLM.text("Continued", "text-xai-final"))

    const input = yield* s.runPrompt(pending)

    expect(s.compactAttempts).toHaveLength(1)
    expect(s.modelResolutions).toBe(1)
    expect(s.requests).toHaveLength(2)
    expect(s.requests.every((request) => request.model === xaiResponsesModel)).toBeTrue()
    expect(userTexts(s.requests[0])).not.toContain(pending)
    expect(userTexts(s.requests[1])).toContain(pending)
    expect(hasRemoteCheckpoint(s.requests[1]!, remoteCheckpoint("cmp_xai_auto"))).toBeTrue()
    expect(yield* SessionInbox.find(s.db, input.id)).toBeUndefined()
    expect(yield* s.context).toMatchObject([
      {
        type: "compaction",
        status: "completed",
        remote: [{ type: "compaction", id: "cmp_xai_auto", encrypted_content: encrypted }],
      },
      Expected.user(pending),
      { type: "assistant", finish: "stop" },
    ])
  })

  scenario("re-compacts an existing xAI checkpoint", function* (s) {
    yield* s.llm.push(TestLLM.textWithUsage("Earlier answer", "text-xai-recompact-earlier", 79_950))
    yield* s.runPrompt("Earlier xAI context ".repeat(180))
    s.currentModel = xaiResponsesModel
    yield* s.llm.push(
      explicitCheckpoint("cmp_xai_first"),
      TestLLM.textWithUsage("First continuation", "text-xai-first-final", 79_950),
    )
    yield* s.runPrompt("First pending xAI turn ".repeat(20))

    s.requests.length = 0
    s.compactAttempts.length = 0
    const pending = "Second pending xAI turn ".repeat(20)
    yield* s.llm.push(explicitCheckpoint("cmp_xai_second"), TestLLM.text("Second continuation", "text-xai-second"))
    const input = yield* s.runPrompt(pending)

    expect(s.compactAttempts).toHaveLength(1)
    expect(s.requests).toHaveLength(2)
    expect(hasRemoteCheckpoint(s.requests[0]!, remoteCheckpoint("cmp_xai_first"))).toBeTrue()
    expect(userTexts(s.requests[0])).not.toContain(pending)
    expect(userTexts(s.requests[1])).toContain(pending)
    expect(yield* SessionInbox.find(s.db, input.id)).toBeUndefined()
    expect(yield* s.context).toContainEqual(
      expect.objectContaining({
        type: "compaction",
        remote: [expect.objectContaining({ id: "cmp_xai_second", encrypted_content: "opaque-cmp_xai_second" })],
      }),
    )
  })

  scenario("uses explicit xAI compaction for a manual request", function* (s) {
    s.currentModel = xaiResponsesModel
    yield* s.llm.push(TestLLM.text("Earlier xAI answer", "text-xai-manual-history"))
    yield* s.runPrompt("Earlier xAI question")
    s.requests.length = 0
    s.modelResolutions = 0
    const hooks = yield* PluginHooks.Service
    ...
    yield* s.llm.push(explicitCheckpoint("cmp_xai_manual"))
    ...
    expect((yield* s.messages).find((message) => message.id === compaction.id)).toMatchObject({
      ...
      status: "completed",
      reason: "manual",
      remote: [{ type: "compaction", id: "cmp_xai_manual", encrypted_content: "opaque-cmp_xai_manual" }],
    })
  })

  scenario("resolves and permits explicit compaction again on a later xAI Step", function* (s) {
    yield* s.llm.push(TestLLM.textWithUsage("Earlier answer", "text-xai-two-step-earlier", 79_950))
    yield* s.runPrompt("Earlier two-Step xAI context ".repeat(180))
    s.currentModel = xaiResponsesModel
    s.requests.length = 0
    s.modelResolutions = 0
    yield* s.llm.push(
      explicitCheckpoint("cmp_xai_step_one"),
      TestLLM.complete(
        { reason: { normalized: "tool-calls" }, usage: { inputTokens: 79_999, nonCachedInputTokens: 79_999 } },
        LLMEvent.toolCall({ id: "call-xai-two-step", name: "echo", input: { text: "continue" } }),
      ),
      explicitCheckpoint("cmp_xai_step_two"),
      TestLLM.text("Two-Step complete", "text-xai-two-step-final"),
    )

    yield* s.runPrompt("Pending two-Step xAI turn ".repeat(20))

    expect(s.compactAttempts).toHaveLength(2)
    expect(s.modelResolutions).toBe(2)
    expect(s.requests).toHaveLength(4)
    expect(s.requests.every((request) => request.model === xaiResponsesModel)).toBeTrue()
    expect(yield* s.context).toContainEqual(
      expect.objectContaining({
        type: "compaction",
        remote: [expect.objectContaining({ id: "cmp_xai_step_two" })],
      }),
    )
  })

  for (const failure of ["404", "malformed", "missing-item"] as const) {
    scenario(`leaves durable history unchanged when xAI explicit compaction returns ${failure}`, function* (s) {
      yield* s.llm.push(TestLLM.textWithUsage("Earlier answer", `text-xai-${failure}-earlier`, 79_950))
      yield* s.runPrompt("Earlier xAI context ".repeat(180))
      s.currentModel = xaiResponsesModel
      s.requests.length = 0
      const pending = yield* s.admit(`Pending xAI ${failure} turn `.repeat(20))
      const before = yield* s.context
      if (failure === "404") s.compactFailure = compact404()
      else if (failure === "malformed") s.compactFailure = malformedCompaction()
      else
        yield* s.llm.push(
          new CompactionResponse({
            replacement: [],
      ...
      expect(yield* s.resume.pipe(Effect.exit)).toMatchObject({ _tag: "Failure" })

      expect(s.compactAttempts).toHaveLength(1)
      expect(s.requests).toHaveLength(failure === "missing-item" ? 1 : 0)
      expect(yield* s.context).toEqual(before)
      expect(yield* SessionInbox.find(s.db, pending.id)).toMatchObject({ id: pending.id, type: "user" })
      const lifecycle = new Set<string>(
        [
          SessionEvent.Compaction.Started.type,
          SessionEvent.Compaction.RemoteItem.type,
          SessionEvent.Compaction.Ended.type,
          SessionEvent.Compaction.Failed.type,
        ].map((type) => Bus.versionedType(type, 1)),
      )
      expect((yield* recordedEventTypes(sessionID)).filter((type) => lifecycle.has(type))).toEqual([])
    })
  }

  scenario("re-previews xAI input when a higher-priority steer arrives", function* (s) {
    s.currentModel = xaiResponsesModel
    const hooks = yield* PluginHooks.Service
    let injected = false
    yield* hooks.register("session", "context", () => {
      ...
      injected = true
      return s.session
        .prompt({ sessionID, text: "Race steer", delivery: "steer", resume: false })
        .pipe(Effect.orDie, Effect.asVoid)
    })
    yield* s.llm.push(TestLLM.text("Steer response", "text-race-steer"), TestLLM.text("Queue response", "text-race-queue"))
    yield* s.session.prompt({ sessionID, text: "Race queue", delivery: "queue", resume: false })

    yield* s.resume

    expect(s.compactAttempts).toHaveLength(0)
    expect(s.requests).toHaveLength(2)
    expect(userTexts(s.requests[0])).toContain("Race steer")
    expect(userTexts(s.requests[0])).not.toContain("Race queue")
    expect(userTexts(s.requests[1])).toContain("Race queue")
  })

  scenario("runs local automatic compaction before preparing the normal request", function* (s) {
    yield* s.llm.push(TestLLM.textWithUsage("Earlier answer", "text-local-hook-earlier", 3_950))
    yield* s.runPrompt("Earlier local context ".repeat(180))
    s.systemBaseline = "Changed before local compaction"
    s.currentModel = compactModel
    const variant = Model.VariantID.make("summary-variant")
    yield* s.bus.publish(SessionEvent.ModelSelected, {
      sessionID,
      model: { id: ID.make(compactModel.id), providerID: Provider.ID.make(compactModel.provider), variant },
    })
    s.requests.length = 0
    const agents = yield* Agent.Service
    ...
    yield* agents.transform((editor) =>
      editor.update(agentID, (agent) => {
        agent.system = "Review the selected Session context."
      }),
    )
    ...
    yield* hooks.register("session", "model.request", (event) =>
      Effect.sync(() => {
        ...
        event.headers["x-compaction-context"] = "preserved"
      }),
    )
    const pending = "Pending local turn ".repeat(20)
    yield* s.llm.push(
      TestLLM.complete(
        {
          reason: { normalized: "stop" },
          providerMetadata: { openai: { responseId: "local-summary-response" } },
        },
        LLMEvent.textDelta({ id: "text-local-hook-summary", text: "## Objective\n- Local summary" }),
      ),
      TestLLM.text("Local continuation", "text-local-hook-final"),
    )

    yield* s.runPrompt(pending)

    expect(s.requests).toHaveLength(2)
    expect(preparedUsers).toHaveLength(1)
    expect(s.requests[0]?.model).toBe(compactModel)
    expect(s.requests[0]?.tools.length).toBeGreaterThan(0)
    expect(s.requests[0]?.system.map((part) => part.text)).toContain("Review the selected Session context.")
    expect(systemTexts(s.requests[0]!)).toContain("Changed before local compaction")
    expect(s.requests[0]?.http?.headers?.["x-compaction-context"]).toBe("preserved")
    expect(s.requests[1]?.http?.headers?.["x-compaction-context"]).toBe("preserved")
    expect(requestAgents).toEqual([Agent.ID.make("compaction"), agentID])
    expect(userTexts(s.requests[0])).toContain("Earlier local context ".repeat(180))
    expect(userTexts(s.requests[0]).at(-1)).toBe(SessionCompaction.buildPrompt(false))
    expect(userTexts(s.requests[0])).not.toContain(pending)
    expect(preparedUsers[0]?.join("\n")).toContain(pending)
    expect(preparedUsers[0]?.join("\n")).not.toContain(SessionCompaction.buildPrompt(false))
    const persisted = {
      model: { id: compactModel.id, providerID: compactModel.provider, variant },
      providerState: { responseId: "local-summary-response" },
    }
    expect((yield* s.messages).find((message) => message.type === "compaction")).toMatchObject(persisted)
    yield* replaySessionProjection(sessionID)
    expect((yield* s.messages).find((message) => message.type === "compaction")).toMatchObject(persisted)
  })

  scenario("keeps an automatic native checkpoint instead of triggering again above the grace", function* (s) {
    s.currentModel = nativeResponsesModel
    const checkpoint = remoteCheckpoint("cmp_auto")
    yield* s.llm.push([
      LLMEvent.providerCompactionStart({}),
      LLMEvent.providerCheckpoint({ reset: true, item: checkpoint }),
      LLMEvent.stepStart({ index: 0 }),
      LLMEvent.textStart({ id: "text-auto-checkpoint" }),
      LLMEvent.textDelta({ id: "text-auto-checkpoint", text: "Checkpointed" }),
      LLMEvent.textEnd({ id: "text-auto-checkpoint" }),
      LLMEvent.stepFinish({
        index: 0,
        reason: { normalized: "stop" },
        usage: { inputTokens: 102_000, nonCachedInputTokens: 102_000 },
      }),
      LLMEvent.finish({ reason: { normalized: "stop" } }),
    ])
    yield* s.runPrompt("Continue with a provider checkpoint")

    expect(s.requests).toHaveLength(1)
    expect(yield* s.context).toMatchObject([
      { type: "compaction", status: "completed", remote: [checkpoint] },
      { type: "assistant", finish: "stop" },
    ])
  })

  scenario("triggers one native checkpoint after the automatic threshold grace", function* (s) {
    s.currentModel = nativeResponsesModel
    const hooks = yield* PluginHooks.Service
    yield* hooks.register("session", "context", (event) =>
      Effect.sync(() => event.system.push(SystemPart.make("ordinary native trigger prefix"))),
    ...
    preserveRulesContext = true
    const checkpoint = remoteCheckpoint("cmp_grace")
    yield* s.llm.push(TestLLM.textWithUsage("Completed above grace", "text-grace", 102_000), remoteTrigger(checkpoint))
    yield* s.runPrompt("Continue above the threshold grace")

    expect(s.requests).toHaveLength(2)
    expect(s.requests[1]?.tools).toEqual([])
    expect(s.requests[1]?.toolChoice).toBeUndefined()
    expect(s.requests[1]?.providerOptions?.compactThreshold).toBeUndefined()
    expect(JSON.stringify(s.requests[0]?.system)).toContain(SessionRulesLocation.Header)
    expect(JSON.stringify(s.requests[1]?.system)).not.toContain(SessionRulesLocation.Header)
    expect(s.requests[1]?.system.map((part) => part.text)).toContain("ordinary native trigger prefix")
    expect(yield* s.context).toContainEqual(expect.objectContaining({ type: "compaction", remote: [checkpoint] }))
  })

  scenario("does not let a prior manual checkpoint suppress a later native grace fallback", function* (s) {
    s.currentModel = nativeResponsesModel
    const manualCheckpoint = remoteCheckpoint("cmp_prior_manual")
    const fallbackCheckpoint = remoteCheckpoint("cmp_after_manual")
    yield* s.llm.push(
      TestLLM.tool("call-manual-boundary", "echo", { text: "active" }),
      remoteTrigger(manualCheckpoint),
      TestLLM.textWithUsage("Continuation above grace", "text-after-manual", 102_000),
      remoteTrigger(fallbackCheckpoint),
    )
    yield* s.admit("Active native work")
    const active = yield* s.resumePaused
    const compaction = yield* s.session.compact({ sessionID })
    yield* active.finish
    ...
    expect((yield* s.messages).find((message) => message.id === compaction.id)).toMatchObject({
      ...
      status: "completed",
      reason: "manual",
      remote: [manualCheckpoint],
    })
    expect(yield* s.context).toContainEqual(
      expect.objectContaining({ type: "compaction", remote: [fallbackCheckpoint] }),
    )
  })

  scenario("recovers one native overflow only after a replayable checkpoint", function* (s) {
    s.currentModel = nativeResponsesModel
    const hooks = yield* PluginHooks.Service
    yield* hooks.register("session", "context", (event) =>
      Effect.sync(() => event.system.push(SystemPart.make("ordinary native trigger prefix"))),
    ...
    preserveRulesContext = true
    const checkpoint = remoteCheckpoint("cmp_overflow")
    yield* s.llm.push(
      [LLMEvent.providerError({ message: "prompt too long", classification: "context-overflow" })],
      remoteTrigger(checkpoint),
      TestLLM.text("Recovered after checkpoint", "text-overflow-recovered"),
    )
    yield* s.runPrompt("Recover native overflow")
    ...
    expect(JSON.stringify(s.requests[1]?.system)).not.toContain(SessionRulesLocation.Header)
    expect(s.requests[1]?.system.map((part) => part.text)).toContain("ordinary native trigger prefix")
    expect(JSON.stringify(s.requests[2]?.system)).toContain(SessionRulesLocation.Header)
    expect(hasRemoteCheckpoint(s.requests[2]!, checkpoint)).toBeTrue()
    expect(yield* s.context).toMatchObject([
      { type: "compaction", status: "completed", remote: [checkpoint] },
      { type: "assistant", finish: "stop" },
    ])
  ...
  scenario("stops native overflow recovery when the trigger has no replayable checkpoint", function* (s) {
    s.currentModel = nativeResponsesModel
    yield* s.llm.push(
      [LLMEvent.providerError({ message: "prompt too long", classification: "context-overflow" })],
      [
      ...
      TestLLM.text("Must not run", "text-unreachable-native-retry"),
    )
    yield* s.admit("Fail native overflow recovery")
    ...
    expect(yield* s.context).toContainEqual(
      expect.objectContaining({
        ...
        status: "failed",
        reason: "auto",
        error: expect.objectContaining({ type: "provider.invalid-output" }),
      }),
    )
    expect(yield* s.context).not.toContainEqual(expect.objectContaining({ summary: "Must not run" }))
  })
  ...
  scenario("reads unresolved stale tool owners from the current assistant suffix", function* (s) {
    ...
    const tool = (id: string, name: string, state: Record<string, unknown>, executed?: boolean) => ({
      type: "tool",
      id,
      name,
      state,
    ...
    const compaction = Schema.decodeUnknownSync(SessionMessage.CompactionCompleted)({
      id: "msg_current_boundary",
      type: "compaction",
      status: "completed",
      reason: "auto",
      summary: "",
    ...
    const older = assistant("msg_stale_before_boundary", [
      tool("repeated", "echo", { status: "running", input: {}, metadata: { source: "old" } }),
    ])
    const terminal = assistant("msg_terminal", [
      tool("completed", "echo", {
        status: "completed",
        input: {},
    ...
    const first = assistant("msg_first_owner", [
      tool("repeated", "echo", { status: "streaming", input: '{"text":"first"}' }),
      tool(
        "child",
        "subagent",
        { status: "running", input: { agent: "general" }, metadata: { sessionID: "ses_child", private: "metadata" } },
        true,
      ),
    ])
    const second = assistant("msg_second_owner", [
      tool("repeated", "shell", { status: "running", input: { command: "pwd" }, metadata: { source: "second" } }),
    ])

    yield* insertHistoryMessage(s.db, sessionID, 1, older)
    yield* insertHistoryMessage(s.db, sessionID, 2, compaction)
    yield* Effect.forEach(
      ...
      (index) =>
        insertHistoryMessage(
          ...
          Schema.decodeUnknownSync(SessionMessage.User)({
            id: `msg_irrelevant_${index}`,
            type: "user",
            text: "irrelevant ".repeat(2_000),
            time: { created: 0 },
          }),
        ),
    )
    yield* insertHistoryMessage(s.db, sessionID, 67, terminal)
    yield* insertHistoryMessage(
      s.db,
      sessionID,
      68,
      assistant("msg_irrelevant_large", [{ type: "text", text: "not a candidate ".repeat(20_000) }]),
    )
    yield* insertHistoryMessage(s.db, sessionID, 69, first)
    yield* insertHistoryMessage(s.db, sessionID, 70, second)

    const pending = yield* SessionHistory.pendingToolCalls(s.db, sessionID)

    expect(pending.map((item) => [item.assistantMessageID, item.tool.id, item.tool.name])).toEqual([
      ["msg_first_owner", "repeated", "echo"],
      ["msg_first_owner", "child", "subagent"],
      ["msg_second_owner", "repeated", "shell"],
    ])
    expect(pending[1]).toMatchObject({
      tool: { executed: true, state: { metadata: { sessionID: "ses_child", private: "metadata" } } },
    })
    const store = yield* SessionStore.Service
    expect(yield* store.context(sessionID)).toHaveLength(69)
  })

  scenario("fails stale settlement reads when the selected completed compaction is malformed", function* (s) {
    const running = Schema.decodeUnknownSync(SessionMessage.Assistant)({
      id: "msg_valid_running_before_malformed_boundary",
      type: "assistant",
      agent: "build",
      model: { id: "fake-model", providerID: "fake" },
      content: [
    ...
    yield* insertHistoryMessage(s.db, sessionID, 1, running)
    yield* s.db.run(sql`
      insert into session_message (id, session_id, type, seq, time_created, time_updated, data)
      values ('msg_malformed_completed_compaction', ${sessionID}, 'compaction', 2, 0, 0, '{"status":"completed","time":{"created":0}}')
    `)

    expect(yield* SessionHistory.pendingToolCalls(s.db, sessionID).pipe(Effect.flip)).toMatchObject({
      _tag: "Session.MessageDecodeError",
      messageID: "msg_malformed_completed_compaction",
    })
  })

  scenario("fails stale settlement reads for malformed non-candidate assistant rows", function* (s) {
    const boundary = Schema.decodeUnknownSync(SessionMessage.CompactionCompleted)({
      id: "msg_valid_boundary",
      type: "compaction",
      status: "completed",
      reason: "auto",
      summary: "",
    ...
    yield* insertHistoryMessage(s.db, sessionID, 1, boundary)
    yield* s.db.run(sql`
      insert into session_message (id, session_id, type, seq, time_created, time_updated, data)
      values ('msg_malformed_non_candidate_assistant', ${sessionID}, 'assistant', 2, 0, 0, '{"content":[]}')
    `)

    expect(yield* SessionHistory.pendingToolCalls(s.db, sessionID).pipe(Effect.flip)).toMatchObject({
      _tag: "Session.MessageDecodeError",
      messageID: "msg_malformed_non_candidate_assistant",
    })
  })

  scenario("checks only new remote compaction boundaries for a settled step", function* (s) {
    ...
    const checkpoint = (id: string) =>
      Schema.decodeUnknownSync(SessionMessage.CompactionCompleted)({
        ...
        summary: "",
        recent: "",
        remote: [remoteCheckpoint(id)],
        time: { created: 0 },
      })
    const current = assistant("msg_remote_boundary_current")
    yield* insertHistoryMessage(s.db, sessionID, 1, checkpoint("cmp_old"))
    yield* insertHistoryMessage(
      s.db,
      sessionID,
      2,
      Schema.decodeUnknownSync(SessionMessage.User)({
        id: "msg_remote_boundary_separator",
        type: "user",
        text: "after the old checkpoint",
        time: { created: 0 },
      }),
    )
    yield* insertHistoryMessage(s.db, sessionID, 3, assistant("msg_remote_boundary_previous"))
    yield* insertHistoryMessage(s.db, sessionID, 4, current)
    // This malformed non-compaction suffix must not be decoded by the boundary query.
    yield* s.db.run(sql`
      insert into session_message (id, session_id, type, seq, time_created, time_updated, data)
      values ('msg_malformed_irrelevant_suffix', ${sessionID}, 'user', 5, 0, 0, '{}')
    `)

    expect(yield* SessionHistory.remoteCompactionForStep(s.db, sessionID, current.id)).toBe(false)

    yield* insertHistoryMessage(s.db, sessionID, 6, checkpoint("cmp_later"))
    expect(yield* SessionHistory.remoteCompactionForStep(s.db, sessionID, current.id)).toBe(true)

    yield* insertHistoryMessage(s.db, sessionID, 7, checkpoint("cmp_before_next_step"))
    const next = assistant("msg_remote_boundary_next")
    yield* insertHistoryMessage(s.db, sessionID, 8, next)
    expect(yield* SessionHistory.remoteCompactionForStep(s.db, sessionID, next.id)).toBe(true)
  })

  scenario("fails closed when a new remote compaction boundary is malformed", function* (s) {
    const current = Schema.decodeUnknownSync(SessionMessage.Assistant)({
      id: "msg_malformed_remote_boundary_current",
      type: "assistant",
      agent: "build",
      model: { id: "fake-model", providerID: "fake" },
      content: [],
    ...
    yield* insertHistoryMessage(s.db, sessionID, 1, current)
    yield* s.db.run(sql`
      insert into session_message (id, session_id, type, seq, time_created, time_updated, data)
      values ('msg_malformed_remote_boundary', ${sessionID}, 'compaction', 2, 0, 0, '{"status":"completed","time":{"created":0}}')
    `)

    expect(yield* SessionHistory.remoteCompactionForStep(s.db, sessionID, current.id).pipe(Effect.flip)).toMatchObject({
      _tag: "Session.MessageDecodeError",
      messageID: "msg_malformed_remote_boundary",
    })
  })
  ...
  scenario("bounds jittered exponential backoff for eligible pre-output failures", function* (s) {
    ...
    const scheduled = yield* retryScheduleQueue(s.bus, sessionID)
    const run = yield* s.resume.pipe(Effect.forkChild)
    yield* s.llm.wait(1)
