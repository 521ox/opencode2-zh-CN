...
const llmLayer = TestLLM.layer({ fallback: TestLLM.text("Review complete", "review") })
const it = testEffect(
  AppNodeBuilder.build(
    LayerNode.group([Session.node, Job.node, SessionContinuation.node, PluginRuntime.providerNode, LocationServiceMap.node]),
    [
...
describe("command subagents", () => {
  for (const fixture of [
    ...
    it.live(`runs ${fixture.name} through recoverable background ownership`, () =>
      Effect.gen(function* () {
        ...
        const markers = yield* jobs.pendingBackground
        expect(markers).toHaveLength(1)
        const marker = markers[0]
        if (!marker || marker.recovery.kind !== "subagent" || !marker.recovery.continuationID)
          return yield* Effect.die("Expected continuation-aware subagent recovery")
        expect(marker).toMatchObject({
          id: SessionContinuation.waiterID(marker.recovery.continuationID),
          status: "running",
          recovery: { parentSessionID: parent.id, childSessionID: child.id, agent: fixture.agent },
        })
        expect(yield* continuations.get(marker.recovery.continuationID)).toMatchObject({
          request: { parentSessionID: parent.id, childSessionID: child.id, agent: fixture.agent },
          state: "bound",
        })
  ...
  it.live("subagent: false overrides subagent mode and the legacy alias", () =>
    Effect.gen(function* () {
      ...
      expect(yield* sessions.get(parent.id)).toMatchObject({ agent: "reviewer", model: { id: "child" } })
...
function project(
  ...
  return Effect.gen(function* () {
    ...
    const definition = {
      ...
      template: "Review $ARGUMENTS: !`bun -e \"process.stdout.write('ready')\"`",
      ...command,
    }
