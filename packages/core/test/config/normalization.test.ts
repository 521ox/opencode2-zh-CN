...
describe("ConfigNormalize", () => {
  ...
  test("merges bounded compaction leaves and retains supported pruning", () => {
    const result = normalized({
      compaction: {
        auto: false,
    ...
    expect(result.encoded.compaction).toEqual({ auto: false, prune: true, keep: { tokens: 2000 }, buffer: 4000 })
