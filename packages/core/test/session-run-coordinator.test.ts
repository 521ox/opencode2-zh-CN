...
describe("SessionRunCoordinator", () => {
  ...
  it.effect("awaitIdle spans a successor woken during terminal settlement", () =>
    Effect.gen(function* () {
      const settling = yield* Deferred.make<void>()
      ...
      const coordinator = yield* SessionRunCoordinator.make({
        drain: () => {
          ...
          return drains === 1
            ? Effect.void
            : Deferred.succeed(successorStarted, undefined).pipe(Effect.andThen(Deferred.await(releaseSuccessor)))
        },
        settled: () =>
          drains === 1
            ? Deferred.succeed(settling, undefined).pipe(Effect.andThen(Deferred.await(releaseSettlement)))
            : Effect.void,
      })
      ...
      yield* Deferred.await(settling)
      const idle = yield* coordinator.awaitIdle("session").pipe(Effect.forkChild)
      // The first owner already exited, so this wake lands in its settlement-window doorbell.
      yield* coordinator.wake("session")
