    ...
    yield* ctx.catalog.transform((evt) => {
      for (const item of evt.provider.list()) {
        ...
        for (const model of item.models.values()) {
          evt.model.update(item.provider.id, model.id, (editor) => {
            if (resourceName && typeof editor.settings?.baseURL === "string")
              editor.settings.baseURL = expandResourceName(
                editor.settings.baseURL,
                resolveResourceName(editor.settings, resourceName) ?? resourceName,
              )
            if (responsesWebSocketCapable(item.provider, editor)) editor.capabilities.responsesWebsockets = true
          })
        }
      }
    })
