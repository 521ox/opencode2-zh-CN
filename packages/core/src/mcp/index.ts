    ...
    Effect.gen(function* () {
      ...
      const refreshPrompts = (name: ServerName, entry: ServerEntry, connection: McpClient.Connection) =>
        connection.prompts().pipe(
          ...
          Effect.andThen(bus.publish(PromptsChanged, { server: name }).pipe(Effect.ignore)),
        )
        ...
        connection.onToolsChanged(() =>
          live(
            refreshTools(name, entry, connection).pipe(
              Effect.andThen(bus.publish(McpEvent.ToolsChanged, { server: name }).pipe(Effect.ignore)),
            ),
          ),
        )
        connection.onPromptsChanged(() => live(refreshPrompts(name, entry, connection)))
        connection.onResourcesChanged(() =>
          live(bus.publish(McpEvent.ResourcesChanged, { server: name }).pipe(Effect.ignore)),
        )
      }
      ...
      const reconcile = Effect.fnUntraced(function* (next: Data) {
        const servers = new Map(next.servers)
