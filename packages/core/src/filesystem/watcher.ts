 ...
 * layer uses `node:fs.watch` for files and immediate entries and `@parcel/watcher` for directories;
 ...
 * acquired watches and routes emitted updates the way the OS watches would.
 */
      ...
      if (input.type === "file" || input.type === "entries") {
        return Effect.sync(() => {
          ...
          const subscription = watch(directory, { recursive: false }, (_event, file) => {
            if (file && !names.has(file.toString())) return
            for (const name of file ? [file.toString()] : names) {
