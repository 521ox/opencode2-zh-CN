...
import { SessionAssistantActiveTable, SessionMessageTable, SessionTable } from "./sql.js"
...
export const get = Effect.fn("SessionStats.get")(function* (input: Input = {}) {
  ...
  const activeAssistants: SessionMessage.Assistant[] = []
  const addAssistant = (row: Omit<MessageRow, "type">) => {
    totals.steps++
      ...
      Effect.gen(function* () {
        const rows = yield* db
          ...
          WHERE message.type IN ('user', 'assistant')
            ...
            AND (session.fork_session_id IS NULL OR message.time_created >= session.time_created)
            AND NOT EXISTS (
              SELECT 1 FROM ${SessionAssistantActiveTable} AS active
        ...
        const activeRows = yield* db
          .select({ message: SessionMessageTable, parentID: SessionTable.parent_id })
          .from(SessionMessageTable)
          .innerJoin(SessionTable, eq(SessionTable.id, SessionMessageTable.session_id))
          .innerJoin(SessionAssistantActiveTable, eq(SessionAssistantActiveTable.message_id, SessionMessageTable.id))
          .where(
            and(
              gte(SessionMessageTable.time_created, range.from),
              lt(SessionMessageTable.time_created, range.to),
              sql`(${SessionTable.fork_session_id} IS NULL OR ${SessionMessageTable.time_created} >= ${SessionTable.time_created})`,
              input.projectID === undefined ? undefined : eq(SessionTable.project_id, input.projectID),
            ),
          )
          .all()
          .pipe(Effect.orDie)
        const hydrated = yield* SessionMessageProjection.hydrate(
          db,
          activeRows.map((row) => row.message),
        )
        hydrated.forEach((message, index) => {
          if (message.type !== "assistant") throw new Error(`Active message ${message.id} is not an assistant`)
          const row = activeRows[index]
          if (row.parentID === null) sessions.add(row.message.session_id)
          else subagents.add(row.message.session_id)
          activeAssistants.push(message)
          addAssistant({
            sessionID: row.message.session_id,
            parentID: row.parentID,
            timeCreated: row.message.time_created,
            providerID: message.model.providerID,
            modelID: message.model.id,
            variant: message.model.variant ?? null,
  ...
  if (toolMode !== "none")
    activeAssistants.forEach((assistant) => {
      assistant.content.forEach((content) => {
        if (content.type !== "tool") return
        addToolStatus(toolTotals, content.state.status, 1)
        if (toolMode !== "detail") return
        ...
        addToolStatus(tool, content.state.status, 1)
        if (content.time.completed)
          tool.durations.push(
...
function rowTokens(row: Pick<MessageRow, "input" | "output" | "reasoning" | "cacheRead" | "cacheWrite">): Tokens {
  return {
    input: row.input ?? 0,
    output: row.output ?? 0,
