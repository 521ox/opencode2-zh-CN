...
export interface Interface {
  ...
  /** Loads active history using an already resolved model selected for this turn. */
  readonly loadResolved: (
    selection: Selection,
    model: SessionRunnerModel.Resolved,
  ) => Effect.Effect<Loaded, SessionRunnerModel.Error>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadResolved = Effect.fn("SessionContext.loadResolved")(function* (
      selection: Selection,
      model: SessionRunnerModel.Resolved,
    ) {
    ...
    const load = Effect.fn("SessionContext.load")(function* (selection: Selection) {
      ...
      return yield* loadResolved(selection, model)
    })

    return Service.of({ select, load, loadResolved, resolveModel, selectTitle, prepare: modelRequests.prepare })
  }),
)
