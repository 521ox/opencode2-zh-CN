  ...
  Effect.gen(function* () {
    ...
    const resolveDestination = Effect.fn("SessionMove.resolveDestination")(function* (
      ...
      yield* Location.Service.pipe(
        Effect.provide(locations.get(destination.location)),
        Effect.scoped,
      ...
      yield* upsertProject(database.db, project).pipe(Effect.orDie)
      return destination
    })

    const sourceUnavailable = Effect.fn("SessionMove.sourceUnavailable")(function* (session: Session.Info) {
      if ((yield* execution.active).has(session.id)) return false
      if (!(yield* fs.isDir(session.location.directory))) return true
      return yield* SessionRunner.Service.use(() => Effect.void).pipe(
    ...
    return Service.of({
      move: Effect.fn("SessionMove.move")(function* (input) {
        ...
        // Only the lease owner may probe or recover this Session's selected instance.
        const owned = execution.claim ? yield* execution.claim(input.sessionID) : false
        const unavailable = owned ? yield* sourceUnavailable(session) : false
        ...
        yield* SessionInbox.serialized(
          ...
          Effect.gen(function* () {
            ...
            const stillOwned = execution.claim ? yield* execution.claim(input.sessionID) : false
            if (
              unavailable &&
              stillOwned &&
