  ...
  test.describe(theme, () => {
    ...
    for (const scenario of [
      ...
      test(`existing session send button: ${scenario.name}`, async ({ page }, testInfo) => {
        ...
        const expected = scenario.accent ? "accent" : "contrast"
        await expectBackground(view.send, expected)
    ...
    test("inventory updates accent registered worktrees; disabled and stop stay neutral", async ({ page }) => {
