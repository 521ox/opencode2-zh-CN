...
describe("server persistence schema", () => {
  test("normalizes legacy server shapes and writes only current server objects", () => {
    ...
    expect(state).toEqual({
      list: [
        ...
        { type: "http", http: { url: "https://flat.example", username: "legacy", password: "first" } },
        {
          type: "http",
          displayName: "Remote",
          label: "Production",
  ...
  async () => {
    ...
    try {
      ...
      expect(root.state[0].list).toEqual([
        { type: "http", http: { url: "https://remote.example", username: "legacy", password: "secret" } },
      ])
      ...
      expect(stored).toContain('"username":"legacy"')
